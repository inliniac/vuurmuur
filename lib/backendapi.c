...
*/
static int
load_plugin(struct vrmr_config *cfg, struct vrmr_list *plugin_list,
        char *plugin_name, struct vrmr_plugin_data **func_ptr)
{
    ...
    vrmr_debug(HIGH, "** start **, plugin_nane: '%s', pluginlist size: '%d'.", plugin_name, plugin_list->len);
...
*/
static int
unload_plugin(struct vrmr_list *plugin_list, char *plugin_name, struct vrmr_plugin_data **func_ptr)
{
    struct vrmr_plugin  *plugin = NULL;
    struct vrmr_list_node         *d_node = NULL;
...
*/
int
vrmr_backends_load(struct vrmr_config *cfg, struct vrmr_ctx *vctx)
{
    textdir_init();

    /* first the SERVICES */
    if (load_plugin(cfg, &vrmr_plugin_list, cfg->serv_backend_name, &vctx->sf) < 0)
        return(-1);
    ...
    /*
        second ZONES
    */
    if (load_plugin(cfg, &vrmr_plugin_list, cfg->zone_backend_name, &vctx->zf) < 0)
        return(-1);
    ...
    /*
        third INTERFACES
    */
    if (load_plugin(cfg, &vrmr_plugin_list, cfg->ifac_backend_name, &vctx->af) < 0)
        return(-1);
    ...
    /*
        last RULES
    */
    if (load_plugin(cfg, &vrmr_plugin_list, cfg->rule_backend_name, &vctx->rf) < 0)
        return(-1);
...
*/
int
vrmr_backends_unload(struct vrmr_config *cfg, struct vrmr_ctx *vctx)
{
    /*
        SERVICES
    */
