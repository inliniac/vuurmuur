...
*/
char
*list_textdir(void *backend, char *name, int *zonetype, int type)
{
    ...
    if(type == VRMR_BT_SERVICES)
    {
        ...
        while(!done)
        {
            ...
            {
                /* open the dir */
                if(!(tb->service_p = vuurmuur_opendir(tb->cfg, dir_location)))
                {
                    vrmr_error(-1, "Error", "unable to open '%s', %s (in: list_textdir, opendir).", dir_location, strerror(errno));
                    return(NULL);
                }
            }
            ...
            if((dir_entry_p = readdir(tb->service_p)) != NULL)
            {
                    ...
                    strcmp(name, "..") != 0)
                {
                    ...
                    if(vrmr_validate_servicename(name, tb->servicename_reg, VRMR_QUIET) == 0)
                    {
                        /* determine the location of the file */
                        if(!(file_location = get_filelocation(backend, name, VRMR_TYPE_SERVICE)))
                            return(NULL);

                        vrmr_debug(HIGH, "service '%s', file: '%s'.", name, file_location);

                        /* now stat it */
                        if(vrmr_stat_ok(tb->cfg, file_location, VRMR_STATOK_WANT_FILE, VRMR_STATOK_QUIET, VRMR_STATOK_MUST_EXIST))
                        {
                            free(file_location);
    ...
    else if(type == VRMR_BT_INTERFACES)
    {
        ...
        while(done == 0)
        {
            ...
            {
                // open the dir
                if(!(tb->interface_p = vuurmuur_opendir(tb->cfg, dir_location)))
                {
                    vrmr_error(-1, "Error", "Unable to open '%s', %s.", dir_location, strerror(errno));
                    return(NULL);
                }
            }
                    ...
                    strlen(dir_entry_p->d_name) > 5)
                {
                    ...
                    )
                    {
                        ...
                        if((strlen(dir_entry_p->d_name) < VRMR_MAX_INTERFACE+5))
                        {
                            ...
                            if(vrmr_validate_interfacename(tb->interface, tb->interfacename_reg) == 0)
                            {
                                *zonetype = VRMR_TYPE_INTERFACE;
                                (void)strlcpy(name, tb->interface, VRMR_MAX_INTERFACE);

                                // determine the location of the file
                                if(!(file_location = get_filelocation(backend, name, VRMR_TYPE_INTERFACE)))
                                    return(NULL);

                                vrmr_debug(HIGH, "interface '%s', file: '%s'.", name, file_location);
                            ...
                            vrmr_debug(HIGH, "'%s' is too long.", dir_entry_p->d_name);
                        }
                    }
                }
            }
                    ...
                    )
                    {
                        ...
                        if((strlen(dir_entry_p->d_name) < MAX_RULE_NAME + 5))
                        {
                            ...
                            *zonetype = VRMR_TYPE_RULE;
                            ...
                            /* determine the location of the file */
                            if(!(file_location = get_filelocation(backend, name, VRMR_TYPE_RULE)))
                                return(NULL);

                            vrmr_debug(HIGH, "rule '%s', file: '%s'.", name, file_location);
        ...
        while(!done)
        {
            ...
            if(tb->host_p != NULL)
            {
                ...
                if((dir_entry_p = readdir(tb->host_p)) != NULL)
                {
                    ...
                    if((strncmp(dir_entry_p->d_name, ".", 1) != 0) && (strcmp(dir_entry_p->d_name, "..") != 0) && (strlen(dir_entry_p->d_name) > 5))
                    {
                        ...
                        vrmr_debug(HIGH, "item: %s, %d.", dir_entry_p->d_name, (int)strlen(dir_entry_p->d_name));
                        ...
                        if(strlen(dir_entry_p->d_name) < VRMR_MAX_HOST + 5)
                        {
                            ...
                            if(vrmr_validate_zonename(cur_zonename, 1, NULL, NULL, NULL, tb->zonename_reg, VRMR_QUIET) == 0)
                            {
                                // determine the location of the file
                                if(!(file_location = get_filelocation(backend, cur_zonename, VRMR_TYPE_HOST)))
                                    return(NULL);
                                vrmr_debug(HIGH, "host '%s', file: '%s'.", cur_zonename, file_location);
            ...
            else if(tb->group_p != NULL)
            {
                ...
                if((dir_entry_p = readdir(tb->group_p)) != NULL)
                {
                    ...
                    if((strncmp(dir_entry_p->d_name, ".", 1) != 0) && (strcmp(dir_entry_p->d_name, "..") != 0) && (strlen(dir_entry_p->d_name) > 5))
                    {
                        vrmr_debug(HIGH, "group name: %s.", dir_entry_p->d_name);
                        ...
                        if(strlen(dir_entry_p->d_name) < VRMR_MAX_HOST + 5)
                        {
                            ...
                            {
                                // determine the location of the file
                                if(!(file_location = get_filelocation(backend, cur_zonename, VRMR_TYPE_GROUP)))
                                    return(NULL);
                                vrmr_debug(HIGH, "group '%s', file: '%s'.", cur_zonename, file_location);
            ...
            else if(tb->network_p != NULL)
            {
                ...
                if((dir_entry_p = readdir(tb->network_p)) != NULL)
                {
                    ...
                    if((strncmp(dir_entry_p->d_name, ".", 1) != 0) && (strcmp(dir_entry_p->d_name, "..") != 0))
                    {
                        ...
                        {
                            // determine the location of the file
                            if(!(file_location = get_filelocation(backend, cur_zonename, VRMR_TYPE_NETWORK)))
                                return(NULL);
                            vrmr_debug(HIGH, "list_textdir: network '%s', file: '%s'.", cur_zonename, file_location);
                            ...
                            if(vrmr_stat_ok(tb->cfg, file_location, VRMR_STATOK_WANT_FILE, VRMR_STATOK_QUIET, VRMR_STATOK_MUST_EXIST))
                            {
                                ...
                                vrmr_debug(HIGH, "list_textdir: '%s' ('%s', '%s').", cur_zonename, tb->cur_network, tb->cur_zone);
            ...
            else if(tb->zone_p != NULL)
            {
                ...
                if((dir_entry_p = readdir(tb->zone_p)) != NULL)
                {
                    ...
                    if((strncmp(dir_entry_p->d_name, ".", 1) != 0) && (strcmp(dir_entry_p->d_name, "..") != 0))
                    {
                        ...
                        /* this is allowed to fail, if it does, is will be NULL, and will be detected above */
                        tb->network_p = vuurmuur_tryopendir(tb->cfg, netdir_location);

                        // lets check against regex
                        if(vrmr_validate_zonename(dir_entry_p->d_name, 1, NULL, NULL, NULL, tb->zonename_reg, VRMR_QUIET) == 0)
                        {
                            // determine the location of the file
                            if(!(file_location = get_filelocation(backend, dir_entry_p->d_name, VRMR_TYPE_ZONE)))
                                return(NULL);
                            vrmr_debug(HIGH, "zone '%s', file: '%s'.", tb->cur_zone, file_location);
            ...
            if(!done && !tb->zone_p)
            {
                ...
                {
                    vrmr_error(-1, "Error", "unable to open directory: %s: %s.", dir_location, strerror(errno));
                    return(NULL);
                }
            }
        }
