 ...
 */
static int
determine_action(struct vrmr_config *cfg, char *query,
        ...
        struct vrmr_rule_options *option, int broadcast)
{
    ...
    else if(action_type == VRMR_AT_REJECT)
    {
        ...
        {
            vrmr_debug(MEDIUM, "reject option: "
                    ...
                    struct vrmr_config *cnf)
{
    ...
    else
    {
        ...
        */
        ...
        {
            if(!(create->service = vrmr_search_service(services, rule_ptr->service)))
            {
        ...
        if(create->ruletype == VRMR_RT_BOUNCE)
        {
            ...
            create->via_int = vrmr_search_interface(interfaces,
    ...
    else
    {
        ...
        if(cnf->bash_out == TRUE && create->description != NULL)
        {
            ...
            if(action_ptr != NULL)
                option_ptr = vrmr_rules_assemble_options_string(rule_ptr->opt, action_ptr);
...
*/
int
vrmr_rules_init_list(struct vrmr_ctx *vctx, struct vrmr_config *cfg, struct vrmr_rules *rules, struct vrmr_regex *reg)
{
    FILE                *fp = NULL;
    int                 retval = 0;
    ...
    if((fp = fopen(cfg->rules_location, "r")))
    {
        ...
        //TODO what? what? what?
            ...
            {
                vrmr_debug(HIGH, "skipping line because its a comment or its empty.");
            }
            /* no comment */
            else
            {
    ...
    else
    {
        ...
        while((vctx->rf->ask(vctx->rule_backend, "rules", "RULE", line, sizeof(line), VRMR_TYPE_RULE, 1)) == 1)
        {
            /* check if the line is a comment */
        //TODO what? what? what?
            if((strlen(line) <= 1) || (line[0] == '#'))
            {
                vrmr_debug(MEDIUM, "skipping line because its a comment or its empty.");
            }
            /* no comment */
            else
            {
...
*/
int
vrmr_rules_parse_line(char *line, struct vrmr_rule *rule_ptr, struct vrmr_regex *reg)
{
    ...
    */
    if(rule_ptr->action == VRMR_AT_PROTECT)
    {
        ...
        */
        ...
        {
            vrmr_debug(HIGH, "now we know who (%s), let get the danger (but first check who).", rule_ptr->who);

            /*
                validate the who-zone
            */
            if(vrmr_validate_zonename(rule_ptr->who, 1, NULL, NULL, NULL, reg->zonename, VRMR_VERBOSE) != 0)
            {
            ...
            */
            ...
            vrmr_debug(HIGH, "protect: keyword against: '%s'", rule_ptr->danger);
    ...
    else
    {
        if(rule_ptr->action != VRMR_AT_SEPARATOR)
        {
            ...
            */
            if(vrmr_validate_servicename(rule_ptr->service, reg->servicename, VRMR_QUIET) != 0)
            {
            ...
            */
            ...
            if(strncasecmp(rule_ptr->from, "firewall", 8) != 0)
            {
                ...
                */
                if(vrmr_validate_zonename(rule_ptr->from, 1, NULL, NULL, NULL, reg->zonename, VRMR_VERBOSE) != 0)
                {
            ...
            */
            ...
            if (strncasecmp(rule_ptr->to, "firewall", 8) != 0) {
                ...
                */
                if(vrmr_validate_zonename(rule_ptr->to, 1, NULL, NULL, NULL, reg->zonename, VRMR_VERBOSE) != 0)
                {
...
*/
char *
vrmr_rules_assemble_rule(struct vrmr_rule *rule_ptr)
{
    char    *line = NULL,
            buf[512] = "",
            *option_ptr = NULL;
    ...
    option_ptr = vrmr_rules_assemble_options_string(rule_ptr->opt, vrmr_rules_itoaction(rule_ptr->action));
    if(option_ptr != NULL)
    {
...
*/
static int
rules_write_file(const struct vrmr_config *cnf, struct vrmr_rules *rules, const char *rulesfile_location)
{
    FILE                *fp = NULL;
    int                 retval = 0;
...
int
vrmr_rules_save_list(struct vrmr_ctx *vctx, struct vrmr_rules *rules, struct vrmr_config *cnf)
{
    int                 result = 0;
    char                *line = NULL,
                        eline[1024] = "";
    ...
    else
    {
        ...
        if(rules->list.len == 0)
        {
            if(vctx->rf->tell(vctx->rule_backend, "rules", "RULE", "", 1, VRMR_TYPE_RULE) < 0)
            {
        ...
        else
        {
            ...
            for(d_node = rules->list.top; d_node ; d_node = d_node->next)
            {
                ...
                if (vctx->rf->tell(vctx->rule_backend, "rules", "RULE", eline, overwrite, VRMR_TYPE_RULE) < 0)
                {
                    vrmr_error(-1, "Internal Error", "rf->tell() failed (in: %s:%d).",
                            __FUNC__, __LINE__);
                    return(-1);
...
*/
int
vrmr_rules_cleanup_list(struct vrmr_rules *rules)
{
    struct vrmr_list_node         *d_node = NULL;
    struct vrmr_rule    *rule_ptr = NULL;
...
*/
int
vrmr_rules_insert_list(struct vrmr_rules *rules, unsigned int place, struct vrmr_rule *rule_ptr)
{
    ...
    vrmr_debug(HIGH, "insert at: %d. (list len is %d), number: %d, action: %s, service: %s, from: %s, to: %s, danger: %s, who: %s, source: %s.",
            place, rules->list.len, rule_ptr->number, vrmr_rules_itoaction(rule_ptr->action),
            rule_ptr->service, rule_ptr->from, rule_ptr->to, rule_ptr->danger,
            rule_ptr->who, rule_ptr->source);


    /* if we insert into empty list, we always insert at the top */
    if(rules->list.len == 0)
    {
        vrmr_debug(HIGH, "inserting into an empty list. Setting place to 1 (place was: %d).", place);
        place = 1;
    }
    ...
    {
        vrmr_debug(HIGH, "place > rules_list->len (%d, %d). Setting place to %d.", place, rules->list.len, rules->list.len + 1);
    ...
    if(place == 1)
    {
        ...
        vrmr_debug(HIGH, "vrmr_list_prepend succes, now update numbers (place: %d)", place);
    ...
    */
    for(d_node = rules->list.top; d_node ; d_node = d_node->next)
    {
        ...
        vrmr_debug(HIGH, "entry: %s %s %s %s %s", vrmr_rules_itoaction(listrule_ptr->action), listrule_ptr->service, listrule_ptr->danger, listrule_ptr->who, listrule_ptr->source);

        if(listrule_ptr->number == place - 1)
        {
            vrmr_debug(HIGH, "place to insert: place: %d, %s %s %s %s %s", place, vrmr_rules_itoaction(listrule_ptr->action), listrule_ptr->service, listrule_ptr->danger, listrule_ptr->who, listrule_ptr->source);
            ...
            vrmr_debug(HIGH, "vrmr_list_insert_after succes, now update numbers (place: %d)", place);

            vrmr_rules_update_numbers(rules, place - 1, 1);

            /* set the number */
            vrmr_debug(HIGH, "vrmr_list_insert_after succes, now set rule_ptr->number to place: %d.", place);
        ...
        {
            vrmr_debug(HIGH, "not the right place: %d, %s %s %s %s %s", place, vrmr_rules_itoaction(listrule_ptr->action), listrule_ptr->service, listrule_ptr->danger, listrule_ptr->who, listrule_ptr->source);
        }
    }
...
char *
vrmr_rules_assemble_options_string(struct vrmr_rule_options *opt,
                                const char *action)
{
    ...
    if(action_type == VRMR_AT_PORTFW || action_type == VRMR_AT_DNAT)
    {
        if(opt->listenport == 1)
        {
            ports_ptr = vrmr_list_to_portopts(&opt->ListenportList, "listenport");
            if(ports_ptr != NULL)
            {
        ...
        if(opt->remoteport == 1)
        {
            ports_ptr = vrmr_list_to_portopts(&opt->RemoteportList, "remoteport");
            if(ports_ptr != NULL)
            {
                if(strlcat(options, ports_ptr, sizeof(options)) >= sizeof(options))
                {
...
*/
int
vrmr_rules_compare_options(struct vrmr_rule_options *old_opt, struct vrmr_rule_options *new_opt, char *action)
{
    char    *old_str = NULL,
            *new_str = NULL;
    int     retval = 0;
...
*/
void *
vrmr_search_rule(struct vrmr_rules *rules, struct vrmr_rule *searchrule_ptr)
{
    struct vrmr_list_node         *d_node = NULL;
    struct vrmr_rule    *listrule_ptr = NULL;
                ...
                if(strcmp(listrule_ptr->service, searchrule_ptr->service) == 0)
                {
                    ...
                    if(strcmp(listrule_ptr->from, searchrule_ptr->from) == 0)
                    {
                        ...
                        if(strcmp(listrule_ptr->to, searchrule_ptr->to) == 0)
                        {
                            ...
                            if(vrmr_rules_compare_options(listrule_ptr->opt, searchrule_ptr->opt, vrmr_rules_itoaction(listrule_ptr->action)) == 0)
                            {
                                return(listrule_ptr);
                            }
                        }
...
static int
parse_option(const char *curopt, struct vrmr_rule_options *op)
{
    ...
    else if(strncmp(curopt, "loglimit", strlen("loglimit")) == 0)
    {
        ...
        vrmr_debug(MEDIUM, "loglimit: %d, logburst %d.", op->loglimit, op->logburst);
    }
    /* limit */
    ...
    else if(strncmp(curopt, "redirectport", strlen("redirectport")) == 0)
    {
        ...
        vrmr_debug(MEDIUM, "redirectport: %d, %s", op->redirectport, portstring);
    }
    /* nfmark */
    ...
    else if(strncmp(curopt, "in_max", strlen("in_max")) == 0)
    {
                ...
                strcmp(unit_string, "mbps") == 0)
        {
            ...
            vrmr_debug(MEDIUM, "value_string %s unit_string %s", value_string, unit_string);
        } else {
...
*/
int
vrmr_rules_read_options(const char *optstr, struct vrmr_rule_options *op)
{
    int     retval = 0,
            trema = 0;
    char    curopt[512];
    ...
    vrmr_debug(HIGH, "options: '%s', strlen(optstr): %d", optstr, (int)strlen(optstr));
...
struct vrmr_rule *
rules_create_protect_rule(char *action, /*@null@*/ char *who, char *danger, /*@null@*/ char *source)
{
    struct vrmr_rule    *rule_ptr = NULL;
...
*/
int
vrmr_rules_chain_in_list(struct vrmr_list *list, char *chainname)
{
    char        *str = NULL;
    struct vrmr_list_node *d_node = NULL;
...
*/
int
vrmr_rules_get_custom_chains(struct vrmr_rules *rules)
{
    struct vrmr_rule    *rule_ptr = NULL;
    struct vrmr_list_node         *d_node = NULL;
    char                *str = NULL;
    ...
    for(d_node = rules->list.top; d_node; d_node = d_node->next)
    {
        ...
        if(rule_ptr->opt != NULL)
        {
            if(rule_ptr->opt->chain[0] != '\0')
            {
                ...
                if(vrmr_rules_chain_in_list(&rules->custom_chain_list, rule_ptr->opt->chain) == 0)
                {
...
static int
vrmr_rules_get_system_chains_per_table(char *tablename,
...
*/
int
vrmr_rules_get_system_chains(struct vrmr_rules *rules, struct vrmr_config *cnf, int ipv)
{
    /* safety */
    if(cnf == NULL || rules == NULL)
    {
    ...
    if(vrmr_rules_get_system_chains_per_table("filter",
                &rules->system_chain_filter, cnf, ipv) < 0)
        return(-1);
    if(vrmr_rules_get_system_chains_per_table("mangle",
                &rules->system_chain_mangle, cnf, ipv) < 0)
        return(-1);

    if (ipv == VRMR_IPV4) {
        if(vrmr_rules_get_system_chains_per_table("nat",
                    &rules->system_chain_nat, cnf, ipv) < 0)
            return(-1);
    }
    //if(vrmr_rules_get_system_chains_per_table("raw", &rules->system_chain_raw, cnf) < 0)
    //    return(-1);
...
*/
int
vrmr_rules_encode_rule(char *rulestr, size_t size)
{
    char    line[1024] = "";
    size_t  i = 0,
            x = 0;
...
int
vrmr_rules_decode_rule(char *rulestr, size_t size)
{
    char    line[1024] = "";
    size_t  i = 0,
            x = 0;
...
*/
int
vrmr_rules_determine_ruletype(struct vrmr_rule *rule_ptr)
{
    int ruletype;
...
*/
struct vrmr_rule *
vrmr_rules_remove_rule_from_list(struct vrmr_rules *rules, unsigned int place, int updatenumbers)
{
    struct vrmr_rule *rule_ptr = NULL;
    struct vrmr_list_node *d_node = NULL;
    ...
    vrmr_debug(LOW, "start: place: %d, updatenumbers: %d, listsize: %d", place, updatenumbers, rules->list.len);
    ...
    for(d_node = rules->list.top; d_node ; d_node = d_node->next)
    {
        ...
        vrmr_debug(HIGH, "rule_ptr->number: %d, place: %d", rule_ptr->number, place);

        if(rule_ptr->number != place)
            continue;

        vrmr_debug(HIGH, "now we have to remove (query_ptr->number: %d, place: %d)", rule_ptr->number, place);
...
*/
void
vrmr_rules_update_numbers(struct vrmr_rules *rules, unsigned int place, int action)
{
    struct vrmr_rule    *rule_ptr = NULL;
    struct vrmr_list_node         *d_node = NULL;
    unsigned int        i = 0;
    ...
    vrmr_debug(HIGH, "Update higher (or equal) than: %d, action = %d. (list len is %d)", place, action, rules->list.len);
...
void
vrmr_rules_free_options(struct vrmr_rule_options *opt)
{
    struct vrmr_list_node     *d_node = NULL;
