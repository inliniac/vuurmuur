    ...
    for (i = 0; vrmr_icmp_types[i].type != -1; i++) {
        if (vrmr_icmp_types[i].type == type) {
            if (vrmr_icmp_types[i].has_code == 1) {
                /*
                    if we called with code == -1, it means we don't want to know
                   about the code
                */
                if (code == -1) {
                ...
                */
                        ...
                        if (vrmr_icmp_codes[k].code == code) {
                            if (only_code == 0) {
                                snprintf(name, size, "%s(%s)",
                                        ...
                                        vrmr_icmp_codes[k].short_name);
                            } else {
                                (void)strlcpy(name,
                                        vrmr_icmp_codes[k].short_name, size);
                            }
                ...
                snprintf(name, size, "%s(err:%d)",
                        vrmr_icmp_types[i].short_name, code);
                return (0);
            } else {
                (void)strlcpy(name, vrmr_icmp_types[i].short_name, size);
                return (0);
...
int vrmr_list_icmp_types(int *type, int *has_code, int *number)
{
    if (vrmr_icmp_types[*number].type != -1) {
        ...
        // vrmr_icmp_types[*number].type, vrmr_icmp_types[*number].short_name);
...
int vrmr_list_icmp_codes(int type, int *code, int *number)
{
    ...
    if (vrmr_icmp_codes[*number].type < type) {
        ...
        while (vrmr_icmp_codes[*number].type != type &&
                vrmr_icmp_codes[*number].type != -1)
            *number = *number + 1;

        // fprintf(stdout, "number: %d (%d %d)\n", *number, type,
        // vrmr_icmp_codes[*number].type);
    }
