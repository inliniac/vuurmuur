...
pipe_iptables_command(const int debuglvl, char *table, char *chain, char *cmd)
{
    ...
    */
    if(vrmr_pipe_command(debuglvl, &conf, str, PIPE_VERBOSE) < 0)
    {
        ...
        struct vrmr_interfaces *interfaces, struct vrmr_iptcaps *iptcap, int ipv)
{
    ...
    for (d_node = interfaces->list.top; d_node; d_node = d_node->next)
    {
        ...
        {
#ifdef IPV6_ENABLED
            if (ipv == VR_IPV6 && !vrmr_interface_ipv6_enabled(debuglvl, iface_ptr))
                continue;
#endif
        ...
        struct vrmr_iptcaps *iptcap, int ipv)
{
    ...
    */
    ...
    if (conf.vrmr_check_iptcaps == TRUE && ipv == VR_IPV4 && iptcap->match_connmark == FALSE)
        memset(connmark, 0, sizeof(connmark));
#ifdef IPV6_ENABLED
    if (conf.vrmr_check_iptcaps == TRUE && ipv == VR_IPV6 && iptcap->match_ip6_connmark == FALSE)
        memset(connmark, 0, sizeof(connmark));
#endif
...
pre_rules(const int debuglvl, /*@null@*/RuleSet *ruleset, struct vrmr_interfaces *interfaces, struct vrmr_iptcaps *iptcap)
{
    ...
    /* syn-flooding protection */
    if (pre_rules_synlimit(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_synlimit(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif

    /* udp-flooding protection */
    if (pre_rules_udplimit(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_udplimit(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif

    if (pre_rules_newaccept(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_newaccept(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif
    ...
    if (pre_rules_newqueue(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_newqueue(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif

    if (pre_rules_nfqueue(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_nfqueue(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif
    ...
    if (pre_rules_conntrack_invalid(debuglvl, ruleset, iptcap, VR_IPV4) < 0)
        retval = -1;
#ifdef IPV6_ENABLED
    if (pre_rules_conntrack_invalid(debuglvl, ruleset, iptcap, VR_IPV6) < 0)
        retval = -1;
#endif
        ...
        struct vrmr_iptcaps *iptcap, int ipv)
{
    ...
    if (ruleset == NULL)
    {
        if (ipv == VR_IPV4) {
            ...
            snprintf(cmd, MAX_PIPE_COMMAND, "%s --flush SYNLIMIT",
                    conf.iptables_location);
            result = vrmr_pipe_command(debuglvl, &conf, cmd, PIPE_VERBOSE);
            if(result < 0)
                retval = -1;
        ...
        struct vrmr_iptcaps *iptcap, int ipv)
{
    ...
    if (ruleset == NULL) {
        if (ipv == VR_IPV4) {
            /* first flush the chain */
            snprintf(cmd, MAX_PIPE_COMMAND, "%s --flush UDPLIMIT",
                    conf.iptables_location);
            result = vrmr_pipe_command(debuglvl, &conf, cmd, PIPE_VERBOSE);
            if (result < 0)
                retval = -1;
    ...
    */
    ...
    if(from_if_ptr->device_virtual_oldstyle == TRUE)
    {
        ...
        if(conf.vrmr_check_iptcaps == FALSE || iptcap->target_log == TRUE)
        {
            ...
            if(process_rule(debuglvl, ruleset, VR_IPV4, TB_FILTER, CH_ANTISPOOF, cmd, 0, 0) < 0)
                return(-1);
        }
...
create_block_rules(const int debuglvl, /*@null@*/RuleSet *ruleset, struct vrmr_blocklist *blocklist)
{
    ...
    for(d_node = blocklist->list.top; d_node; d_node = d_node->next)
    {
        ...
        if(process_rule(debuglvl, ruleset, VR_IPV4, TB_FILTER, CH_BLOCKLIST, cmd, 0, 0) < 0)
            retval=-1;
