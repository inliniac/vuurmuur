...
script_rename(const int debuglvl, VuurmuurScript *vr_script)
{
    ...
    */
    ...
    if(vr_script->type == TYPE_ZONE)
    {
        if(zf->rename(debuglvl, zone_backend, vr_script->name, vr_script->set, TYPE_ZONE) < 0)
        {
    ...
    {
        if(zf->rename(debuglvl, zone_backend, vr_script->name, vr_script->set, TYPE_NETWORK) < 0)
        {
    ...
    {
        if(zf->rename(debuglvl, zone_backend, vr_script->name, vr_script->set, TYPE_HOST) < 0)
        {
    ...
    {
        if(zf->rename(debuglvl, zone_backend, vr_script->name, vr_script->set, TYPE_GROUP) < 0)
        {
    ...
    {
        if(sf->rename(debuglvl, serv_backend, vr_script->name, vr_script->set, TYPE_SERVICE) < 0)
        {
    ...
    {
        if(af->rename(debuglvl, ifac_backend, vr_script->name, vr_script->set, TYPE_INTERFACE) < 0)
        {
    ...
    {
        if(rf->rename(debuglvl, rule_backend, vr_script->name, vr_script->set, TYPE_RULE) < 0)
        {
