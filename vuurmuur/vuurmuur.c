                    ...
                    &option_index)) != -1 )
    {
        switch(optch)
        {
            ...
            case 'L' :
                ...
                /* loglevel */
                if(strlcpy(vctx.conf.loglevel, optarg, sizeof(vctx.conf.loglevel)) > sizeof(vctx.conf.loglevel))
                {
...
/* TODO: is this really that serious? */
                ...
                sigterm_count == 0)
            {
                ...
                */
                if(vctx.conf.dynamic_changes_check == TRUE && vctx.interfaces.dynamic_interfaces == TRUE)
                {
                    dynamic_wait_time++;
