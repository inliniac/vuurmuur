 ...
 *
 * These are the 'same':
 * /sbin/tc class add dev eth2 parent 4:2 classid 4:12 htb rate 8192kbit ceil
        ...
        struct rule_scratch *rule, struct shape_rule *shape_rule)
{
    ...
    for (d_node = rule->shaperulelist.top; d_node; d_node = d_node->next) {
        ...
        if (shaping_rulecmp(listrule, shape_rule) == 1) {
            free(shape_rule);
            return (0);
        }
    }
        ...
        uint16_t class, char *device, char *cmd)
{
    ...
    if (shaping_ruleinsert(rule, shape_rule) < 0)
        return (-1);
        ...
        struct vrmr_interfaces *interfaces, /*@null@*/ struct rule_set *ruleset)
{
    ...
    for (d_node = interfaces->list.top; d_node != NULL; d_node = d_node->next) {
        ...
        if (strcmp(iface_ptr->device, "") != 0 &&
            ...
            snprintf(cmd, sizeof(cmd),
                    "%s qdisc del dev %s root 2> /dev/null > /dev/null",
                    cnf->tc_location, iface_ptr->device);

            vrmr_debug(NONE, "cmd \"%s\"", cmd);

            if (shaping_process_rule(cnf, ruleset, cmd) < 0)
                return (-1);
        }
    }
     ...
     */
    if (ruleset) {
        if (shaping_process_rule(cnf, ruleset, "true") < 0)
            return (-1);
    }
        ...
        /*@null@*/ struct rule_set *ruleset)
{
    ...
    iface_rate =
            shaping_convert_rate(iface_ptr->bw_out, iface_ptr->bw_out_unit);

    /* tc class add dev ppp0 parent 1: classid 1:1 htb rate 512kbit */
    snprintf(cmd, sizeof(cmd),
            "%s class add dev %s parent %u: classid %u:1 htb rate %ukbit",
    ...
    for (d_node = interfaces->list.top; d_node != NULL; d_node = d_node->next) {
        ...
        if (iface_ptr != inner_iface_ptr && /* don't add a class for yourself */
                vrmr_is_shape_interface(inner_iface_ptr) == 1) {
            rate = shaping_convert_rate(
                    inner_iface_ptr->bw_in, inner_iface_ptr->bw_in_unit);
            if (iface_rate < rate)
                rate = iface_rate;

            /* tc class add dev ppp0 parent 1: classid 1:1 htb rate 512kbit */
            snprintf(cmd, sizeof(cmd),
                    "%s class add dev %s parent %u: classid %u:%u htb rate "
                    "%ukbit",
                    cnf->tc_location, iface_ptr->device,
                    iface_ptr->shape_handle, iface_ptr->shape_handle,
                    inner_iface_ptr->shape_handle, rate);
        ...
        struct vrmr_interfaces *interfaces, /*@null@*/ struct rule_set *ruleset)
{
    ...
    for (d_node = interfaces->list.top; d_node != NULL; d_node = d_node->next) {
        iface_ptr = d_node->data;
        vrmr_debug(NONE, "interface %s", iface_ptr->name);

        if (vrmr_is_shape_interface(iface_ptr) == 1) {
    ...
    for (d_node = interfaces->list.top; d_node != NULL; d_node = d_node->next) {
        ...
        if (vrmr_is_shape_interface(iface_ptr) == 1) {
            snprintf(cmd, sizeof(cmd),
                    "%s qdisc add dev %s root handle %u: htb default %u",
                    cnf->tc_location, iface_ptr->device,
                    iface_ptr->shape_handle, handle);
            ...
            if (shaping_setup_interface_classes(
                        cnf, interfaces, iface_ptr, ruleset) < 0)
                return (-1);
        }
    }
        ...
        struct vrmr_interface *iface_ptr, uint32_t rate, char *unit)
{
    ...
    kbit_rate = shaping_convert_rate(rate, unit);

    vrmr_debug(NONE, "kbit rate %u", kbit_rate);
        ...
        struct vrmr_interfaces *interfaces, struct vrmr_rules *rules)
{
        ...
        if (rule_ptr->active == TRUE) {
            ...
            if (rule_ptr->opt != NULL && (rule_ptr->opt->bw_in_min > 0 ||
                ...
                if (d_node_iface != NULL) {
                    for (; d_node_iface != NULL;
                            d_node_iface = d_node_iface->next) {
                        iface_ptr = d_node_iface->data;
                        vrmr_debug(NONE, "FROM iface_ptr->name %s, rate %u %s",
                                iface_ptr->name, rule_ptr->opt->bw_in_min,
                                rule_ptr->opt->bw_in_min_unit);

                        if (shaping_add_rate_to_iface(iface_ptr,
                                    rule_ptr->opt->bw_in_min,
                                    rule_ptr->opt->bw_in_min_unit) < 0)
                            return (-1);
                    }
                }
                d_node_iface = NULL;
            ...
            if (rule_ptr->opt != NULL &&
                ...
                if (d_node_iface != NULL) {
                    for (; d_node_iface != NULL;
                            d_node_iface = d_node_iface->next) {
                        iface_ptr = d_node_iface->data;
                        vrmr_debug(NONE, "TO iface_ptr->name %s, rate %u %s",
                                iface_ptr->name, rule_ptr->opt->bw_out_min,
                                rule_ptr->opt->bw_out_min_unit);

                        if (shaping_add_rate_to_iface(iface_ptr,
                                    rule_ptr->opt->bw_out_min,
                                    rule_ptr->opt->bw_out_min_unit) < 0)
                            return (-1);
                    }
                }
                d_node_iface = NULL;
    ...
    for (d_node_iface = interfaces->list.top; d_node_iface != NULL;
        ...
        if (vrmr_is_shape_interface(iface_ptr) == 1) {
            rate = shaping_convert_rate(
                    iface_ptr->bw_out, iface_ptr->bw_out_unit);

            vrmr_debug(NONE,
                    "total rate %u, total rules %u, rules using default rate "
                    "%u",
                    iface_ptr->total_shape_rate, iface_ptr->total_shape_rules,
                    iface_ptr->total_default_shape_rules);

            /* over commit */
            if (iface_ptr->total_shape_rate > rate) {
                vrmr_warning(VR_WARN,
                        "bandwidth over committed on interface %s: %ukbit > "
                        "%ukbit.",
                        iface_ptr->name, iface_ptr->total_shape_rate, rate);

                /* the default rate will be the max interface rate / number of
                 * total rules */
                iface_ptr->shape_default_rate =
                        rate / iface_ptr->total_shape_rules;
            }
            /* no shaping rules at all: use a simple default */
                ...
                /* the default rate is max interface rate minus already
                 * explictly commited rate
                 * devided by the number of rules using the default rate */
                iface_ptr->shape_default_rate =
                        (rate - iface_ptr->total_shape_rate) /
                        iface_ptr->total_default_shape_rules;
            }

            vrmr_debug(NONE, "default rate on %s is %ukbit", iface_ptr->name,
                    iface_ptr->shape_default_rate);
        }
    }
        ...
        struct vrmr_interfaces *interfaces, /*@null@*/ struct rule_set *ruleset)
{
    ...
    for (d_node = interfaces->list.top; d_node != NULL; d_node = d_node->next) {
        ...
        if (vrmr_is_shape_interface(iface_ptr) == 1) {
            ...
            /* tc class add dev ppp0 parent 1:1 classid 1:100 htb rate 15kbit
             * ceil 512kbit prio 3 tc qdisc add dev ppp0 parent 1:100 handle
             * 100: sfq perturb 10 */
            snprintf(cmd, sizeof(cmd),
                    "%s class add dev %s parent %u:1 classid %u:%u htb rate "
                    "%ukbit ceil %ukbit prio 3", /* TODO prio should
                                                    configurable */
                    cnf->tc_location, iface_ptr->device,
                    iface_ptr->shape_handle, iface_ptr->shape_handle, handle,
                    iface_ptr->shape_default_rate, rate);
            ...
            snprintf(cmd, sizeof(cmd),
                    "%s qdisc add dev %s parent %u:%u handle %u: sfq perturb "
                    "10",
                    cnf->tc_location, iface_ptr->device,
                    iface_ptr->shape_handle, handle, handle);
        ...
        char *rate_unit, uint32_t ceil, char *ceil_unit, uint8_t prio)
{
    ...
    if (vrmr_is_shape_interface(shape_iface_ptr) == 0)
        return (0);

    vrmr_debug(NONE, "shape on interface %s (handle %u)", shape_iface_ptr->name,
            shape_iface_ptr->shape_handle);

    if (vrmr_is_shape_interface(class_iface_ptr) == 1) {
        class_handle = class_iface_ptr->shape_handle;

        vrmr_debug(NONE, "class of interface %s (handle %u)",
                class_iface_ptr->name, class_iface_ptr->shape_handle);
    }

    /* convert rates to kbit */
    rate = shaping_convert_rate(rate, rate_unit);
    ceil = shaping_convert_rate(ceil, ceil_unit);
    vrmr_debug(NONE, "rate %u, ceil %u", rate, ceil);

    /* use defaults for unused settings */
    if (prio == 0)
        prio = 3;
    if (rate == 0)
        rate = shape_iface_ptr->shape_default_rate;
    if (ceil == 0)
        ceil = shaping_convert_rate(
                shape_iface_ptr->bw_out, shape_iface_ptr->bw_out_unit);
    ...
    /* tc class add dev eth0 parent 3:3 classid 3:160 htb rate 5mbit ceil 10mbit
     * prio 1 tc qdisc add dev eth0 parent 3:160 handle 127: sfq perturb 10
     */
    snprintf(cmd, sizeof(cmd),
            "%s class add dev %s parent %u:%u classid %u:%u htb rate %ukbit "
            "ceil %ukbit prio %u",
            cnf->tc_location, shape_iface_ptr->device,
            shape_iface_ptr->shape_handle, class_handle,
            shape_iface_ptr->shape_handle, class, rate, ceil, prio);
    ...
    snprintf(cmd, sizeof(cmd),
            "%s qdisc add dev %s parent %u:%u handle %u: sfq perturb 10",
            cnf->tc_location, shape_iface_ptr->device,
            shape_iface_ptr->shape_handle, class, class);
